				     ...
				     struct camss_async_subdev *csd)
{
	...
	/*
	 * Most SoCs support both D-PHY and C-PHY standards; accept either.
	 */
	switch (vep.bus_type) {
	...
	/* Get the remote (sensor) endpoint handle. */
	remote_ep = fwnode_graph_get_remote_endpoint(ep);
	if (!remote_ep) {
		dev_err(dev, "No remote endpoint found for %pfw\n", ep);
		return -ENODEV;
	}
	...
	if (ret) {
		dev_err(dev, "Failed to parse remote endpoint\n");
		return ret;
	}

	/* The local (CSIPHY) and remote (sensor) ends must agree. */
	if (vep.bus_type != remote_vep.bus_type) {
		dev_err(dev, "Bus type mismatch! Local (CSI-PHY): %u, Remote (Sensor): %u\n",
